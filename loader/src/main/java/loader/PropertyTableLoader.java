...
abstract class PropertyTableLoader extends Loader{
	...
	PropertyTableLoader(final String hdfsInputDirectory, final String databaseName, final SparkSession spark,
						boolean isPartitioned, String tableName){
		super(hdfsInputDirectory, databaseName, spark);
	 ...
	 *
	 * @param valueColumnName The column name of the values field. The object column for a WPT, and subject column
	 *                           for a IWPT
	...
	private void saveTable(final Dataset<Row> dataset, String tableName, Boolean isPartitioned) {
		if (isPartitioned){
			dataset.write().mode(SaveMode.Overwrite).format(table_format).partitionBy(column_name_subject).saveAsTable(tableName);
	 ...
	 * context but different in a case-sensitive one. For instance:
	 ...
	 * @param propertiesCardinalities The dataset with all properties and their complexity.
	 ...
	 * @param keyColumnName The column name in the TT that will be the primary key in the PT.
	 * @param valuesColumnName The column name in TT that will contains the names of the values columns in the final PT.
	 * @return A PT dataset.
	 */
	Dataset<Row> createPropertyTableDataset(Map<String,Boolean> propertiesCardinalities, String keyColumnName,
		...
		final PropertiesAggregateFunction aggregator =
				new PropertiesAggregateFunction(propertiesCardinalities.keySet().toArray(new String[0]), COLUMNS_SEPARATOR);
		...
		for (String propertyName : propertiesCardinalities.keySet()){
			...
			final String newProperty = propertiesCardinalities.get(propertyName)
					?
					" " + GROUP_COLUMN + "[" + String.valueOf(propertiesList.size()-1) + "] AS " + getValidHiveName(rawProperty)
					: " " + GROUP_COLUMN + "[" + String.valueOf(propertiesList.size()-1) + "][0] AS " + getValidHiveName(rawProperty);
