...
public class OffsetTest extends JavaDataFrameSuiteBase implements Serializable {
	...
	private void queryOnTT(final DatabaseStatistics statistics, final Dataset<Row> fullDataset)  throws Exception {
		...
		Row row1 = RowFactory.create("B", "<mailto:b@example.org>");
		Row row2 = RowFactory.create("C", "<mailto:c1@example.org>");
		Row row3 = RowFactory.create("C", "<mailto:c2@example.org>");
		Row row4 = RowFactory.create("D", null);
	...
	private void queryOnTT2(final DatabaseStatistics statistics, final Dataset<Row> fullDataset)  throws Exception {
		...
		Row row3 = RowFactory.create("C", "<mailto:c2@example.org>");
		Row row4 = RowFactory.create("D", null);
		Row row5 = RowFactory.create("E", null);
		List<Row> rowList = ImmutableList.of(row1, row2, row3, row4, row5);
		Dataset<Row> expectedResult = spark().createDataFrame(rowList, schema);
