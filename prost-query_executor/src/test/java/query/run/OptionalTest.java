...
public class OptionalTest extends JavaDataFrameSuiteBase implements Serializable {
	...
	private void queryOnTT(final DatabaseStatistics statistics, final Dataset<Row> fullDataset) throws Exception {
		...
		final Query query = new Query(classLoader.getResource("queryTestOptional1.q").getPath(), statistics,
				settings);
	...
	private void queryOnTT2(final DatabaseStatistics statistics, final Dataset<Row> fullDataset) throws Exception {
		...
		final ClassLoader classLoader = getClass().getClassLoader();
		final Query query = new Query(classLoader.getResource("queryTestOptional2.q").getPath(), statistics,
				settings);
		...
		final Row row1 = RowFactory.create("Title1", "<http://example.org/author>", "Author1");
		final Row row2 = RowFactory.create("Title2", null, null);
		final List<Row> rowList = ImmutableList.of(row1, row2);
		final Dataset<Row> expectedResult = spark().createDataFrame(rowList, schema);
		
		//ACTUAL
		final Dataset<Row> actualResult = query.compute().orderBy("title", "author", "name");
		final Dataset<Row> nullableActualResult = sqlContext().createDataFrame(actualResult.collectAsList(),
				actualResult.schema().asNullable());
