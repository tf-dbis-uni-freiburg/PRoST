...
public class FilterTest extends JavaDataFrameSuiteBase implements Serializable {
	...
	private void queryOnTT(final DatabaseStatistics statistics, final Dataset<Row> fullDataset) throws Exception {
		...
		final Query query = new Query(classLoader.getResource("queryTestFilter1.q").getPath(), statistics,
				settings);
	...
	private void queryOnTT2(final DatabaseStatistics statistics, final Dataset<Row> fullDataset) throws Exception {
		...
		final ClassLoader classLoader = getClass().getClassLoader();
		final Query query = new Query(classLoader.getResource("queryTestFilter2.q").getPath(), statistics,
				settings);
		...
		final Row row1 = RowFactory.create("Title2", "40");
		final Row row2 = RowFactory.create("Title3", "30");
	...
	private void queryOnTT3(final DatabaseStatistics statistics, final Dataset<Row> fullDataset) throws Exception {
		...
		final ClassLoader classLoader = getClass().getClassLoader();
		final Query query = new Query(classLoader.getResource("queryTestFilter3.q").getPath(), statistics,
				settings);
		...
		final Row row1 = RowFactory.create("Title1", "50");
		final Row row2 = RowFactory.create("Title4", "20");
		final List<Row> rowList = ImmutableList.of(row1, row2);
		final Dataset<Row> expectedResult = spark().createDataFrame(rowList, schema);
