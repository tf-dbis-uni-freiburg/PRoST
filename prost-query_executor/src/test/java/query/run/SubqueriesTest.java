...
public class SubqueriesTest extends JavaDataFrameSuiteBase implements Serializable {
	...
	private void queryOnTT1(final DatabaseStatistics statistics) throws Exception {
		...
		final Row row1 = RowFactory.create("<http://example.org/bob>", "B. Abc");
		final Row row2 = RowFactory.create("<http://example.org/carol>", "C. Abc");
	...
	private void queryOnTT2(final DatabaseStatistics statistics) throws Exception {
		...
		final Row row1 = RowFactory.create("<http://example.org/carol>", "35");
	...
	private void queryOnTT3(final DatabaseStatistics statistics) throws Exception {
		...
		final Row row1 = RowFactory.create("<http://example.org/bob>", "25");
		final Row row2 = RowFactory.create("<http://example.org/carol>", "30");
		final List<Row> rowList = ImmutableList.of(row1, row2);
		final Dataset<Row> expectedResult = spark().createDataFrame(rowList, schema);
