...
public class SingleTriplePatternTest extends JavaDataFrameSuiteBase implements Serializable {
	...
	private void queryOnTT(final DatabaseStatistics statistics, final Dataset<Row> fullDataset) throws Exception {
		...
		final List<String> data = new ArrayList<>();
	    	...
	    	data.add("Title2");
	    	// DataFrame
	    	final Dataset<Row> singleColResult = spark().createDataset(data, Encoders.STRING()).toDF();
	    	final Dataset<Row> expectedResult = singleColResult.selectExpr("split(value, ',')[0] as title");
		//ACTUAL
	...
	private void queryOnTT2(final DatabaseStatistics statistics, final Dataset<Row> fullDataset) throws Exception {
		...
		final Row row1 = RowFactory.create("<http://example.org/book1>", "Title1"); //"Science"
		...
		//ACTUAL
		final Dataset<Row> actualResult = query.compute().orderBy("book", "title");
		final Dataset<Row> nullableActualResult = sqlContext().createDataFrame(actualResult.collectAsList(),
				actualResult.schema().asNullable());
