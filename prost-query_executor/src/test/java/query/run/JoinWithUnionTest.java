...
public class JoinWithUnionTest extends JavaDataFrameSuiteBase implements Serializable {
	...
	private void queryOnTT(final DatabaseStatistics statistics, final Dataset<Row> fullDataset) throws Exception {
		...
		final Query query = new Query(classLoader.getResource("queryTestJoinWithUnion1.q").getPath(), statistics,
				settings);
		...
		final Row row3 = RowFactory.create("Title3");
	...
	private void queryOnTT2(final DatabaseStatistics statistics, final Dataset<Row> fullDataset) throws Exception {
		...
		final ClassLoader classLoader = getClass().getClassLoader();
		final Query query = new Query(classLoader.getResource("queryTestJoinWithUnion2.q").getPath(), statistics,
				settings);	
		...
		final Row row1 = RowFactory.create(null, "Title2");
		final Row row2 = RowFactory.create(null, "Title4");
		final Row row3 = RowFactory.create("Title1", null);
		final Row row4 = RowFactory.create("Title3", null);
		final List<Row> rowList = ImmutableList.of(row1, row2, row3, row4);
		final Dataset<Row> expectedResult = spark().createDataFrame(rowList, schema);
		
		//ACTUAL
		final Dataset<Row> actualResult = query.compute().orderBy("x", "y");
		final Dataset<Row> nullableActualResult = sqlContext().createDataFrame(actualResult.collectAsList(),
				actualResult.schema().asNullable());
	...
	private void queryOnTT3(final DatabaseStatistics statistics, final Dataset<Row> fullDataset) throws Exception {
		...
		final ClassLoader classLoader = getClass().getClassLoader();
		final Query query = new Query(classLoader.getResource("queryTestJoinWithUnion3.q").getPath(), statistics,
				settings);
